...
int main(){
	...
	if(select == 1){  // ค่า select เท่ากับ 1
		...
		if (stcount > 0)
			{ 	Sort_by_id (st,stcount); // เนีบงข้อมุลตามรหัสนศ.
				Print_all_student(st,stcount);
			}
	}
	else if(select == 5){// ค่า select เท่ากับ 5
		if (stcount > 0)
			{ 	Search_Score(st,stcount); // เรียงข้อมูลตามช่วงที่กำหนด
			}

	}
	else if(select == 6){// ค่า select เท่ากับ 6
		if (stcount > 0 && count.A > 0 && count.Bp > 0 && count.B > 0 && count.Cp > 0 && count.C > 0 && count.Dp > 0 
			&& count.D > 0 & count.F > 0) // เช็คว่ามีค่าคนที่ได้เกรดต่างๆหรือไม่ก่อนแสดงผล
			Search_Grade(st,stcount); // ถ้ามีก็ให้แสดงผล
		else
			printf("\t\t\t\t\tDon't Have data that you want to Show. please Calculate Grade\n"); // ไม่มีบอกให้ผู้ใช้ไปคำนวณมาก่อน
			

	}
	else if(select == 7)// ค่า select เท่ากับ 6
		if(stcount > 0) 
			Search_Name(st,stcount); // หาตามชื่อ keyword บางส่วน
		
	else
		printf("\t\t******   End of Program   ******"); // จบโปรแกรม
	}while(select != 0);
	return 0;
}